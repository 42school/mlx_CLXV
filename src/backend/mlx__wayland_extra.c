...
static int	mlx__wayland_extra_set_mouse(void)
{
  static int	warned = 0;

  /* Core Wayland deliberately has no pointer-warp request: a client
     cannot move the cursor, only the compositor (or the user) can.
     mlx_mouse_move() therefore always fails on this backend. */
  if (warned == 0)
    {
      fprintf(stderr, "Mlx - Wayland: mlx_mouse_move() is not supported, "
	      "core Wayland has no pointer-warp protocol\n");
      warned = 1;
    }
  return (-1);
}
...
int	mlx__wayland_extra(mlx_backend_hooks_param_t *param)
{
  ...
  if (param->action == MLX_BE_MOUSE_SET)
    return (mlx__wayland_extra_set_mouse());
